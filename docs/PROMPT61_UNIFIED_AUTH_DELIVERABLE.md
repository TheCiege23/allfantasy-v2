...
- No changes to NextAuth provider config, credentials authorize, or session callbacks; no new auth “services” beyond the above and the existing register/verify/phone flows.

---

## 4. Frontend Auth Flow Updates

- **Signup** (`app/signup/page.tsx`): Reads `next` from URL via `useSearchParams`; uses `getRedirectAfterSignup(next)` for redirect after successful signup (and after verification when applicable). “Go to Sign In” and “Already have an account? Sign in” use `loginUrlWithIntent(redirectAfterSignup)` so post-login redirect matches signup intent. Phone verification redirect includes `returnTo` so verify page can send user to the same destination.
...
| Sign Up button | LoginContent | `Link` to `/signup` (no intent; could add callbackUrl for consistency) | ✅ |
...
**Fix applied**: Signup now respects `next` and passes intent through to login link so post-signup and post-login redirects align. Social “planned” buttons are clickable and show a clear message. Forgot password supports both email and SMS with a clear step flow and SMS code + new password form.
