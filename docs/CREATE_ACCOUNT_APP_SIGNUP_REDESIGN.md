...
- **Apple OAuth bug on `/login`** (`components/auth/SocialLoginButtons.tsx` calling `signIn('apple')` unconditionally despite no real credentials) is a live latent bug, documented above, intentionally not touched — fixing it means changing the login page, which is out of this ticket's scope.
