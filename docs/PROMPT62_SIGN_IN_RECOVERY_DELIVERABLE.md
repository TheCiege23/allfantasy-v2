...
- **ProviderPendingFlow**: Dedicated route `app/auth/provider-pending/page.tsx` for providers that are not configured yet, so buttons remain clickable and intentional.
...
- **Email path**: Enter email → POST `/api/auth/password/reset/request` with `type: 'email'` → 6-digit code sent via Resend → enter code + new password + confirm → POST `/api/auth/password/reset/confirm` with `email` + `code` + `newPassword` → success → redirect to login with `reset=1`.
...
- **Unconfigured / planned** (Google/Apple when not set, Facebook, Instagram, X, TikTok): Buttons remain **clickable**. On click, users are routed to `/auth/provider-pending` with provider context and preserved callback destination. No dead buttons.
...
- **Password reset**: Request/confirm routes support both email-code and SMS-code reset. Resend code reuses POST `/api/auth/password/reset/request` with the active method and identifier (email or phone); backend rate limit applies.
...
| Save new password (email code) | ForgotPasswordClient | `handleConfirmCode` → confirm API with email + code + newPassword | ✅ |
| Google button | SocialLoginButtons | When enabled: `signIn('google', { callbackUrl })`; else route to provider pending flow | ✅ |
| Apple button | SocialLoginButtons | When enabled: `signIn('apple', { callbackUrl })`; else route to provider pending flow | ✅ |
...
- **Forgot password by email**: Request → email code → code + new password confirm in forgot-password flow → redirect to login with reset=1.
- **Forgot password by SMS**: Choose SMS → phone → send code → enter code + new password → resend code available → submit → success → redirect.
- **Social buttons**: All six providers have click handlers; unconfigured/planned route to provider-pending page with clear setup status.
...
- [x] Forgot password by email: request → receive code → verify code → save new password → success → redirect.
...
- **Recovery**: Forgot password offers Email or SMS. Both methods send a 6-digit code; user verifies code and then saves a new password in the same flow. “Resend code” is available for both methods. Confirm API supports email+code or phone+code. On success, user is redirected to login with `reset=1` so a success banner can be shown.

- **Social**: Buttons call `signIn(provider, { callbackUrl })` when the provider is configured; otherwise they route to a dedicated provider-pending flow so UX remains polished and testable. One account and one session are used across WebApp, Bracket, and Legacy.
