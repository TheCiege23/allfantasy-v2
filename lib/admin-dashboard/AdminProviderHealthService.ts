...
export async function getAdminProviderHealthRows(): Promise<AdminProviderHealthRow[]> {
  ...
  return [
    providerRow({
      ...
      dataCategories: ["teams", "fixtures", "live scores", "group standings", "knockout results"],
      consumedBy: ["World Cup sync cron", "World Cup scoring", "leaderboard", "Chimmy DB context"],
      storage: [
        "world_cup_teams",
    ...
    providerRow({
      ...
      storage: ["chat_gifs", "chat_messages metadata"],
      importedRows: null,
      costProtection: ["server-side resolver preferred", "clean fallback when missing"],
      note: hasAnyEnv(["VITE_KLIPY_API_KEY"]) ? "Legacy VITE_ key is present; prefer KLIPY_API_KEY server-side." : "No live probe made.",
      calls,
      rates,
      sync,
    }),
