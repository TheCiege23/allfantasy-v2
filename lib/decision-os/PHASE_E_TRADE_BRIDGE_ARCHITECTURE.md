...
permanent principles (§0.1). Phases **E.1 + E.2 + E.3 + E.4 BUILT** (read-only, shadow-only). **Amendment 2026-06-29
...
**E.2 (Trade Memo on Canonical Assets)**, **E.3 (TradeWorldResolver)**, and **E.4 (Trade Shadow Parity)** are
...
diffs surfaced, no fake parity). 14 E.4 tests GREEN (74 ticket-suite GREEN), zero new type errors. **Next: E.5**
— the read-only ADP/position port seam that feeds `MarketContext` (lifting parity toward GREEN), then live
shadow-parity validation against `theciege24`'s real imported leagues, plus the proposal-time canonical-snapshot
persistence design. Every E phase remains read-only + shadow-only + flag-gated; cutover (G), legacy retirement
(H), and licensing (I) are later, separately governed phases.*
