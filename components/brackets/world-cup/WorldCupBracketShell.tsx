...
export default function WorldCupBracketShell({ initialView, challenge, defaultTab = "picks" }: { initialView?: WorldCupChallengeView; challenge?: WorldCupChallengeView | any; defaultTab?: Tab }) {
  ...
  async function persistPick(match: WorldCupMatchView, side: "home" | "away") { const selectedTeamId = side === "home" ? match.homeTeamId : match.awayTeamId, selectedSlotKey = side === "home" ? match.homeSlotKey : match.awaySlotKey, selectedTeamName = side === "home" ? match.homeTeamName : match.awayTeamName; setPicks((cur) => [...cur.filter((p) => p.matchId !== match.id), { id: `optimistic-${match.id}`, matchId: match.id, round: match.round, selectedTeamId, selectedSlotKey, selectedTeamName, pointsAwarded: 0, isCorrect: null, lockedAt: null }]); setSaveState("saving"); try { const res = await fetch(`/api/brackets/world-cup/${view.challenge.id}/picks`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ picks: [{ matchId: match.id, selectedTeamId, selectedSlotKey }] }) }); const data = await res.json(); if (!res.ok) throw new Error(data.error ?? "Failed"); const nextView = normalizeWorldCupView(data.view ?? data.challenge ?? data); setView(nextView); setPicks(nextView.picks); setSaveState("saved") } catch { setSaveState("error") } }
