...
function PlayerCard({ player, lens, onAskChimmy }) {
  ...
  const confClass =
    player.confidence >= 75 ? styles.confHigh : player.confidence >= 55 ? styles.confMid : styles.confLow
  ...
  return (
    <div className={styles.playerCard} style={{ borderLeftColor: color }}>
      <div className={styles.pcHeader}>
        ...
        <div className={styles.pcRecCol}>
          <div className={styles.pcRec} style={{ color }}>
            {rec}
          </div>
          {player.status !== 'Active' && <span className={styles.badgeInjury}>⚠ {player.status}</span>}
        </div>
      </div>
      ...
      <div className={styles.pcFooter}>
        ...
        <button type="button" className={styles.btnAskChimmy} onClick={() => onAskChimmy(player)}>
          Ask Chimmy ↗
        </button>
      </div>
    </div>
...
function severityDotClass(stylesObj, severity) {
  const s = String(severity || 'low').toLowerCase()
  if (s.startsWith('h')) return stylesObj.dotHigh
  if (s.startsWith('m')) return stylesObj.dotMedium
  return stylesObj.dotLow
}
...
export default function StartSitPopup({ isOpen, onClose, userId }) {
  ...
  const [chatMessages, setChatMessages] = useState([
    {
      role: 'chimmy',
      content:
        "Hey! I'm Chimmy. Select your league above and ask me anything about your lineup — I have live access to player projections, injuries, weather, and matchup data.",
    },
  ])
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`/api/start-sit/leagues?userId=${encodeURIComponent(userId ?? '')}`, {
          credentials: 'include',
        })
        const data = await res.json()
        const sportKey = sport === 'all' ? null : sport
        const list = sportKey ? data[sportKey] ?? [] : Object.values(data).flat()
  ...
  const loadAll = useCallback(async () => {
    ...
    const [rosterRes, injuryRes, weatherRes, matchupRes] = await Promise.allSettled([
      fetch(`/api/start-sit/roster?${params}`, { credentials: 'include' }).then((r) => r.json()),
      fetch(`/api/start-sit/injuries?sport=${encodeURIComponent(sport)}`, { credentials: 'include' }).then((r) =>
        r.json()
      ),
      fetch(`/api/start-sit/weather?sport=${encodeURIComponent(sport)}`, { credentials: 'include' }).then((r) =>
        r.json()
      ),
      fetch(`/api/start-sit/matchups?sport=${encodeURIComponent(sport)}&week=${encodeURIComponent(week)}`, {
        credentials: 'include',
      }).then((r) => r.json()),
    ])
    ...
    if (rosterData.length > 0) {
      ...
      setStatusInfo(
        issues.length > 0
          ? {
              title: `${issues.length} injury flag${issues.length > 1 ? 's' : ''} detected`,
              sub: `${issues.map((p) => p.name).join(', ')} — review before locking`,
              icon: '⚠',
              color: '#f5a623',
            }
          : {
              title: 'Lineup looks good',
              sub: `No critical issues for ${leagueName}`,
              icon: '⚡',
              color: '#00d4aa',
            }
      )
  ...
  const sendChimmy = useCallback(
    async (msg) => {
      if (!msg.trim()) return
      setChatMessages((p) => [...p, { role: 'user', content: msg }])
      setChatLoading(true)
      const historyForApi = [...chatHistory, { role: 'user', content: msg }]
      ...
      try {
        const res = await fetch('/api/start-sit/chimmy', {
          ...
          body: JSON.stringify({
            ...
            context: { sport, leagueName, lens, format, week, roster, injuries, weather, matchups },
          }),
        })
        const data = await res.json()
        const reply = data.reply ?? "Sorry, I couldn't generate a response right now."
        setChatMessages((p) => [...p, { role: 'chimmy', content: reply }])
        setChatHistory([...historyForApi, { role: 'assistant', content: reply }])
        ...
        setChatMessages((p) => [
          ...p,
          {
            role: 'chimmy',
            content:
              'AI temporarily unavailable. Check your API key configuration in Vercel environment variables.',
          },
        ])
      }
  ...
  const handleAskAboutPlayer = useCallback(
    (player) => {
  ...
  const confColor =
    overallConf == null ? '#9ba3bf' : overallConf >= 75 ? '#00d4aa' : overallConf >= 60 ? '#f5a623' : '#f06060'

  if (!isOpen) return null

  const emoji = SPORT_EMOJI[sport] ?? SPORT_EMOJI.all
      ...
      <div className={styles.modal} role="dialog" aria-modal="true" aria-label="Start/Sit Tactical Decision Room">
        <div className={styles.modalHeader}>
          ...
          <div className={styles.headerRight}>
            ...
            <button type="button" className={styles.btnRefresh} onClick={loadAll} title="Refresh all data">
              ↻
            </button>
        ...
        <div className={styles.body}>
          <div className={styles.controlsBar}>
            {[
              ...
              },
              {
                ...
                el: (
                  <select
                    className={`${styles.ctrlSelect} ${styles.leagueSelect}`}
                    value={leagueId}
                    onChange={(e) => {
                      const o = leagues.find((l) => l.id === e.target.value)
                      setLeagueId(e.target.value)
                      setLeagueName(o?.name ?? '')
                    }}
                    data-testid="start-sit-league"
                  >
                    {leagues.map((l) => (
          ...
          <div className={styles.lensesRow}>
            {LENS_OPTIONS.map((o) => (
              <button
                key={o.id}
                type="button"
                className={`${styles.lensBtn} ${lens === o.id ? styles.active : ''}`}
          ...
          <div className={styles.statusBanner}>
            <div className={styles.statusLeft}>
              <div
                ...
                style={{ borderColor: statusInfo.color, background: `${statusInfo.color}22` }}
              >
                {statusInfo.icon}
              </div>
              <div>
                <div className={styles.statusTitle} style={{ color: statusInfo.color }}>
                  {statusInfo.title}
                </div>
                <div className={styles.statusSub}>
                  {statusInfo.sub || `${leagueName} · ${SPORT_LABELS[sport]} · ${format}`}
                </div>
              </div>
            </div>
          ...
          <div className={styles.mainGrid}>
            <div className={styles.leftCol}>
              <div className={styles.panel}>
                ...
                {loading.injuries ? (
                  <div className={styles.feedLoading}>Connecting to injury / news ingest…</div>
                ) : (
                  <div className={styles.feedList}>
                    {injuries.map((n, i) => (
                      <div key={i} className={styles.newsItem}>
                        <div className={`${styles.dot} ${severityDotClass(styles, n.severity)}`} />
                    ...
                    {weather.map((w, i) => (
                      <div key={i} className={styles.weatherCard}>
                        ...
                        <div className={styles.wcRight}>
                          <div className={styles.wcConditions}>
                            {w.temp} · {w.wind}
                          </div>
                          <div className={styles.wcImpact} style={{ color: w.impactColor }}>
                            Impact: {w.impact}
                          </div>
                        </div>
                      </div>
                    ...
                    {matchups.map((m, i) => (
                      <div key={i} className={styles.matchupRow}>
                        <div className={styles.matchupHeader}>
                          ...
                          <span
                            className={styles.matchupRankLabel}
                            style={{
                              color: m.score >= 70 ? '#00d4aa' : m.score >= 45 ? '#f5a623' : '#f06060',
                            }}
                          >
                            {m.rankLabel}
                          </span>
                        ...
                        <div className={styles.matchupBarWrap}>
                          <div
                            className={styles.matchupBarFill}
                            style={{
                              width: `${m.score}%`,
                              background: m.score >= 70 ? '#00d4aa' : m.score >= 45 ? '#f5a623' : '#f06060',
                            }}
                          />
                        </div>
                      </div>
              ...
              <div className={styles.panel}>
                <div className={styles.rosterHeader}>
                  <div className={styles.sectionTitle} style={{ marginBottom: 0 }}>
                    {emoji} Roster Start/Sit Analysis
                  </div>
                ...
                {loading.roster ? (
                  <div className={styles.rosterLoading}>
                    ...
                    <span className={styles.loadingSource}>
                      Rolling Insights → API-Sports → ClearSports → TheSportsDB → Sleeper
                    </span>
                  </div>
                ) : roster.length === 0 ? (
          ...
          <div className={`${styles.panel} ${styles.chimmySection}`}>
            <div className={styles.chimmyHeader}>
              ...
              <div className={styles.quickBtns}>
                {[
                  ...
                  <button key={label} type="button" className={styles.quickBtn} onClick={() => sendChimmy(prompt)}>
                    {label}
                  </button>
                ))}
              </div>
              ...
              {chatLoading && (
                <div className={`${styles.msg} ${styles.msgChimmy}`}>
                  ...
                  <div className={styles.msgBody}>
                    <span className={styles.spinner} /> Analyzing with AI engine...
                  </div>
                </div>
              )}
            </div>
            ...
            <div className={styles.chimmyInputRow}>
              ...
              />
              <button type="button" className={styles.btnPrimary} onClick={handleSendInput} disabled={chatLoading}>
                Ask ↗
              </button>
            </div>
          </div>
