...
/**
 * SavedRecStaleCompare
 *
 * Side-by-side comparison between a saved recommendation's payload and a fresh
 * Chimmy response for the same context.
 * Shows "what changed" at a field level and a Chimmy verdict on the delta.
 */

import { ArrowRight, TrendingDown, TrendingUp, Minus } from 'lucide-react'
import type { UnifiedSavedRecommendation } from '@/lib/chimmy-actions/AIActionModel'

export interface SavedRecStaleCompareProps {
  ...
  /** Optional label for the fresh side */
  freshLabel?: string
  className?: string
}

// Derive human-readable label from a camelCase/snake_case key
...
function diffPayloads(
  ...
  for (const key of allKeys) {
    ...
    if (!changed) continue // Only show changed fields
    ...
    else if (typeof s === 'number' && typeof f === 'number') {
      direction = f > s ? 'up' : f < s ? 'down' : 'neutral'
    }

    result.push({ key, saved: s, fresh: f, changed, direction })
  }
...
export default function SavedRecStaleCompare({
  saved,
  freshPayload,
  freshLabel = 'Current Advice',
  className = '',
}: SavedRecStaleCompareProps) {
  ...
  return (
    <div className={`rounded-xl border border-amber-500/20 bg-amber-500/5 overflow-hidden ${className}`}>
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_20px_minmax(0,1fr)] items-center gap-2 px-4 py-2 border-b border-amber-500/10 bg-white/[0.02]">
        <span className="text-[10px] text-white/40 uppercase tracking-wide">Saved ({
      ...
      {diffs.length === 0 ? (
        ...
        <div className="divide-y divide-white/[0.04]">
          {diffs.map((d) => (
            <div
              key={d.key}
              className="grid grid-cols-[minmax(0,1fr)_20px_minmax(0,1fr)] items-center gap-2 px-4 py-2.5"
            >
              {/* Saved value */}
