    ...
    <div className={`space-y-4 rounded-2xl bg-black/40 p-6 md:p-8 ${className}`}>
      ...
      <ul className="space-y-3">
        {rivalries.map((r) => (
          <li
            key={r.id}
            className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-white/10 bg-white/5 p-4"
          >
            <span className="font-semibold text-white md:text-lg">{r.managerAName}</span>
            <span className="text-amber-400">vs</span>
            <span className="font-semibold text-white md:text-lg">{r.managerBName}</span>
            <span className="w-full text-right text-sm text-zinc-500 md:w-auto">
