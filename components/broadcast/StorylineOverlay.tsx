    ...
    <div className={`space-y-4 rounded-2xl bg-black/40 p-6 md:p-8 ${className}`}>
      ...
      <ul className="space-y-3">
        {storylines.map((s) => (
          ...
          >
            <p className="font-semibold text-white md:text-lg">{s.headline}</p>
            {s.summary && <p className="mt-1 text-sm text-zinc-400 md:text-base">{s.summary}</p>}
            <p className="mt-2 text-xs text-zinc-500">{s.dramaType.replace(/_/g, ' ')}</p>
          </li>
        ))}
      </ul>
    </div>
