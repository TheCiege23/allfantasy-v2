...
/**
 * Dashboard V2 Phase 2.1 — Global Command Center Hero. Only the 'global' context exists today
 * (Commissioner/Team Focus are Phase 2.3/2.4); this component's `variant`-driven copy/art switch
 * is intentionally not built yet — see the Dashboard V2 architecture doc for the full model.
 */
export function GlobalCommandCenterHero({
  userName,
  leagues,
  selectedLeagueId,
  selectedLeague,
  ...
  return (
    <WarRoomCard className="relative overflow-hidden p-5" accentBorder="rgba(34,211,238,0.15)">
      <div
        ...
        style={{
          background: 'radial-gradient(ellipse 80% 60% at 50% -10%, rgba(34,211,238,0.16) 0%, transparent 70%)',
        }}
      />
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="text-[11px] font-bold uppercase tracking-widest text-cyan-400/60">
              {t('dashboard.warroom.hero.globalEyebrow')}
            </p>
            {urgentTodayCount > 0 ? (
          ...
          <h1 className="mt-1 text-[24px] font-black leading-tight tracking-tight text-white sm:text-[28px]">
            {t('dashboard.warroom.hero.globalHeadline')}
          </h1>

          {/* League Scope selector + current-context indicator. Scoped to this Dashboard page only —
              a platform-wide scope provider is a later, separate initiative (see the architecture doc). */}
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <label className="sr-only" htmlFor="league-scope-selector">
              {t('dashboard.warroom.hero.scopeLabel')}
            </label>
            <select
              id="league-scope-selector"
            ...
            <span className="text-[11px] text-white/40">
              {selectedLeague
                ? tInterpolate('dashboard.warroom.hero.contextViewing', { league: selectedLeague.name })
                : t('dashboard.warroom.hero.contextAllLeagues')}
            </span>
          </div>
        </div>
      </div>
