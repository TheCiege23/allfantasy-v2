...
export function DynastyWarRoomPanel({ leagueId }: { leagueId: string }) {
  ...
  if (loading) {
    return (
      ...
      >
        <Loader2 className="h-4 w-4 animate-spin text-violet-300" /> Loading Dynasty AF Legacy…
      </div>
    )
  }
  if (error || !context) {
    return (
      ...
      >
        {error ?? 'Dynasty AF Legacy is unavailable for this league.'}
      </div>
    )
  }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-sm font-bold text-white">Dynasty AF Legacy</h2>
