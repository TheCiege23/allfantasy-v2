...
import { ChevronRight, Globe2, Plus, Sparkles, Trophy, Users } from "lucide-react"
import type { Session } from "next-auth"

// Static Bracket-Pool Sports catalog (Step 3: static sport tiles).
// Pure server-rendered visual showcase — no DB, no client island, no useSession.
// `live: true` routes to the existing /brackets/world-cup home. Others are
// shown as "Coming Soon" placeholders linking to /brackets/discover so
// visitors can still find active pools.
const BRACKET_SPORTS = [
  { key: "SOCCER", label: "Soccer",            short: "SOC",   badge: "SC", live: true,  note: "FIFA World Cup 2026", href: "/brackets/world-cup" },
  { key: "NFL",    label: "NFL",               short: "NFL",   badge: "FB", live: false, note: "Playoff Challenge",   href: "/brackets/discover" },
  { key: "NBA",    label: "NBA",               short: "NBA",   badge: "BK", live: false, note: "Playoff Challenge",   href: "/brackets/discover" },
  { key: "NHL",    label: "NHL",               short: "NHL",   badge: "HK", live: false, note: "Playoff Challenge",   href: "/brackets/discover" },
  { key: "MLB",    label: "MLB",               short: "MLB",   badge: "BS", live: false, note: "Playoff Challenge",   href: "/brackets/discover" },
  { key: "NCAAB",  label: "NCAA Basketball",   short: "NCAAB", badge: "CB", live: false, note: "March Madness",       href: "/brackets/discover" },
  { key: "NCAAF",  label: "NCAA Football",     short: "NCAAF", badge: "CF", live: false, note: "College Playoff",     href: "/brackets/discover" },
] as const
...
export default async function BracketsHomePage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--bg)" }}>
      ...
      {/* ══════════════════════════════════════════════
          STEP 3 — STATIC SPORT TILES
          Server-rendered showcase of all bracket-pool sports.
          No client islands, no Prisma, no useSession.
      ══════════════════════════════════════════════ */}
      <div className="mx-auto max-w-5xl px-4 pb-12 sm:px-6">
        <section
          ...
          style={{
            ...
            boxShadow: "0 24px 56px -32px color-mix(in srgb, var(--accent-cyan) 24%, transparent)",
            backdropFilter: "blur(14px)",
          }}
          aria-labelledby="brackets-sports-heading"
        >
          <div className="mb-4 flex flex-wrap items-end justify-between gap-2">
            <div>
              <div
                className="mb-2 inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em]"
                style={{
                  background: "color-mix(in srgb, var(--accent-cyan) 12%, transparent)",
                  borderColor: "color-mix(in srgb, var(--accent-cyan) 28%, transparent)",
                  color: "var(--accent-cyan-strong)",
              ...
              <h2
                id="brackets-sports-heading"
                className="text-lg font-bold sm:text-xl"
                style={{ color: "var(--text)" }}
              >
                Bracket pools across every major sport
              </h2>
              <p className="mt-1 text-xs sm:text-sm" style={{ color: "var(--muted)" }}>
                Soccer is live now for the 2026 World Cup. NFL, NBA, NHL, MLB, and college brackets roll out on each league's playoff calendar.
              </p>
            </div>
            <span
          ...
          >
            {BRACKET_SPORTS.map((s) => {
              const liveAccent = "var(--accent-cyan-strong)"
              const liveAccentSoft = "color-mix(in srgb, var(--accent-cyan) 18%, transparent)"
              const mutedSoft = "color-mix(in srgb, var(--muted) 14%, transparent)"
              return (
                <Link
                  key={s.key}
                  href={s.href}
                  data-testid={`bracket-sport-tile-${s.key}`}
                  data-live={s.live ? "true" : "false"}
                  aria-label={s.live ? `${s.label} — live` : `${s.label} — coming soon`}
                  className="group relative flex flex-col gap-2 rounded-xl border p-3 transition hover:opacity-95 hover:scale-[1.01]"
                  style={{
                    borderColor: s.live
                      ? "color-mix(in srgb, var(--accent-cyan) 32%, var(--border))"
                      : "var(--border)",
                    background: s.live
                      ? "color-mix(in srgb, var(--accent-cyan) 8%, var(--panel))"
                      : "color-mix(in srgb, var(--panel) 80%, transparent)",
                    opacity: s.live ? 1 : 0.78,
                  }}
                >
                  <div className="flex items-center gap-2">
                    <span
                      ...
                      style={{
                        background: s.live ? liveAccentSoft : mutedSoft,
                        color: s.live ? liveAccent : "var(--muted)",
                      }}
                      aria-hidden="true"
                    >
                      {s.badge}
                    ...
                    {s.live ? (
                      <span
                        className="ml-auto inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide"
                        style={{
                      ...
                      <span
                        ...
                        style={{ background: mutedSoft, color: "var(--muted)" }}
                      >
                        Soon
                      </span>
                    )}
