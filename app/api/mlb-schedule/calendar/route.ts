/**
 * [NEW] app/api/mlb-schedule/calendar/route.ts
 * GET: Returns combined calendar — real MLB games + fantasy events for a league.
 */
...
import { resolveMlbFantasyWeek } from '@/lib/mlb-schedule'
...
export async function GET(req: NextRequest) {
  ...
  for (let w = startWeek; w <= Math.min(endWeek, startWeek + 12); w++) {
    try {
      weeks.push(await resolveMlbFantasyWeek({
        leagueId, leagueFormatId: formatId, leagueVariant: league.leagueVariant, season, week: w,
      }))
    } catch { /* skip */ }
  }
  ...
  if (minDate && maxDate) {
    const rows = await prisma.gameSchedule.findMany({
      where: { sportType: 'MLB', season, startTime: { gte: new Date(minDate), lte: new Date(maxDate + 'T23:59:59Z') } },
      select: { startTime: true, homeTeam: true, awayTeam: true },
      orderBy: { startTime: 'asc' },
    })
    games = rows.map((r) => ({
      date: r.startTime?.toISOString().slice(0, 10) ?? '', homeTeam: r.homeTeam, awayTeam: r.awayTeam, startTime: r.startTime?.toISOString() ?? null,
    }))
  }

  return NextResponse.json({ leagueId, season, sport: 'MLB', leagueType: formatId, leagueVariant: league.leagueVariant, weeks, games })
}
