/**
 * [NEW] app/api/nfl-schedule/calendar/route.ts
 * GET: Returns combined calendar — real NFL games + fantasy events for a league.
 */
...
export async function GET(req: NextRequest) {
  ...
  for (let w = startWeek; w <= Math.min(endWeek, startWeek + 18); w++) {
    try {
      weeks.push(await resolveNflFantasyWeek({ leagueId, leagueFormatId: formatId, leagueVariant: league.leagueVariant, season, week: w }))
    } catch { /* skip */ }
  }
  ...
  if (minDate && maxDate) {
    const rows = await prisma.gameSchedule.findMany({
      where: { sportType: 'NFL', season, startTime: { gte: new Date(minDate), lte: new Date(maxDate + 'T23:59:59Z') } },
  ...
  return NextResponse.json({ leagueId, season, sport: 'NFL', leagueType: formatId, leagueVariant: league.leagueVariant, weeks, games })
}
