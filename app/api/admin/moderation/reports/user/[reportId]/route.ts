...
import { updateUserReportStatus, getUserReportById } from "@/lib/moderation"
...
export async function PATCH(
  ...
  const existing = await getUserReportById(reportId)
  if (!existing) return NextResponse.json({ error: "Report not found" }, { status: 404 })
  const ok = await updateUserReportStatus(reportId, status as ReportStatus)
