/**
 * [NEW] GET: Combined calendar — real NCAAB games + fantasy events for a league.
 */
import { NextRequest, NextResponse } from 'next/server'
import { getServerSession } from 'next-auth'
import { authOptions } from '@/lib/auth'
import { prisma } from '@/lib/prisma'
import { resolveNcaabFantasyWeek } from '@/lib/ncaab-schedule'
...
export async function GET(req: NextRequest) {
  ...
  const weeks = []
  for (let w = startWeek; w <= Math.min(endWeek, startWeek + 25); w++) {
    try { weeks.push(await resolveNcaabFantasyWeek({ leagueId, leagueFormatId: formatId, leagueVariant: league.leagueVariant, season, week: w })) } catch {}
  }
  ...
  if (minDate && maxDate) {
    const rows = await prisma.gameSchedule.findMany({ where: { sportType: 'NCAAB', season, startTime: { gte: new Date(minDate), lte: new Date(maxDate + 'T23:59:59Z') } }, select: { startTime: true, homeTeam: true, awayTeam: true }, orderBy: { startTime: 'asc' } })
  ...
  return NextResponse.json({ leagueId, season, sport: 'NCAAB', leagueType: formatId, leagueVariant: league.leagueVariant, weeks, games })
}
