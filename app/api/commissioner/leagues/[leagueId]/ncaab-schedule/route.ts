/**
 * [NEW] GET/PUT: Commissioner NCAAB schedule config.
 */
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ leagueId: string }> }) {
  ...
  try { previewPlan = await resolveNcaabFantasyWeek({ leagueId, leagueFormatId: (league.leagueType ?? 'redraft') as LeagueFormatId, leagueVariant: league.leagueVariant, season: new Date().getFullYear(), week: 1 }) } catch {}
