/**
 * [NEW] GET/PUT: Commissioner NCAAF scoring config.
 */
import { NextRequest, NextResponse } from 'next/server'
import { getServerSession } from 'next-auth'
import { authOptions } from '@/lib/auth'
import { prisma } from '@/lib/prisma'
import { getLeagueNcaafScoringConfig, saveLeagueNcaafScoringConfig, getNcaafScoringPresets, buildFullNcaafScoringConfig, type NcaafScoringPresetKey } from '@/lib/ncaaf-scoring'
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ leagueId: string }> }) {
  ...
  const config = await getLeagueNcaafScoringConfig(leagueId)
  let isPremium = false
  try { isPremium = (await featureGate.evaluateUserFeatureAccess(session.user.id, 'advanced_scoring')).allowed } catch {}
  return NextResponse.json({ config, presets: getNcaafScoringPresets(), isCommissioner: league.userId === session.user.id, isPremium })
}
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ leagueId: string }> }) {
  ...
  const presetKey = (body.presetKey ?? 'af_default') as NcaafScoringPresetKey
  ...
  await saveLeagueNcaafScoringConfig(leagueId, { presetKey, rules: body.rules ?? buildFullNcaafScoringConfig(presetKey), userId: session.user.id, premiumFeaturesUsed: presetKey === 'custom' })
  return NextResponse.json({ ok: true, config: await getLeagueNcaafScoringConfig(leagueId) })
}
