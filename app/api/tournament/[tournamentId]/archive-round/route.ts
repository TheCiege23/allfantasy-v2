/**
 * [UPDATED] POST: Archive a completed round (closes competitive actions, preserves history).
 */
import { NextRequest, NextResponse } from 'next/server'
import { getServerSession } from 'next-auth'
import { authOptions } from '@/lib/auth'
import { prisma } from '@/lib/prisma'
import { archiveRound } from '@/lib/tournament-mode/TournamentEliminationEngine'
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ tournamentId: string }> }) {
  ...
  const result = await archiveRound(tournamentId, roundIndex)
  await logTournamentAudit(tournamentId, 'archive_round', { actorId: userId, metadata: { roundIndex, ...result } })
