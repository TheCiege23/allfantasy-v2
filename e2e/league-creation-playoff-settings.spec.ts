...
test.describe('@playoff league creation playoff step', () => {
  test('sends playoff defaults from wizard into create payload', async ({ page }) => {
    ...
    await expect(playoffSection.getByText(/playoff settings/i)).toBeVisible()
    await playoffSection.getByLabel('Playoff team count').fill('8')
    await playoffSection.getByLabel('Playoff weeks').fill('3')
    await playoffSection.getByLabel('Playoff start week').fill('14')
    await playoffSection.getByLabel('First-round byes').fill('0')
    await playoffSection.getByLabel('Seeding rules').selectOption('division_winners_first')
    await playoffSection.getByLabel('Bye rules').selectOption('')
    await playoffSection.getByLabel('Reseed behavior').selectOption('reseed_after_round')
    await playoffSection.getByLabel('Consolation plays for').selectOption('cash')
    await playoffSection.getByLabel('Matchup length').fill('1')
    await playoffSection.getByLabel('Total rounds').fill('3')
    await playoffSection.getByLabel('Championship length').fill('1')
    await playoffSection.getByLabel('Consolation bracket').uncheck()
    await playoffSection.getByLabel('Third-place game').check()
    await playoffSection.getByLabel('Toilet bowl').check()
    await playoffSection.getByLabel('Division record').check()
    await page.getByRole('button', { name: /^next$/i }).click()
