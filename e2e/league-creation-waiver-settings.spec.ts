...
test.describe('@waiver league creation waiver step', () => {
  test('sends waiver defaults from wizard into create payload', async ({ page }) => {
    ...
    await expect(waiverSection.getByText(/waiver settings/i)).toBeVisible()
    ...
    await waiverSection.getByLabel('Thu').check()
    await waiverSection.getByLabel('Claim priority').selectOption('priority_lowest_first')
    await waiverSection.getByLabel('Max claims per period').fill('8')
    await waiverSection.getByLabel('Free agent unlock').selectOption('daily')
    await waiverSection.getByLabel('Game lock behavior').selectOption('first_game')
    await waiverSection.getByLabel('Same-day add/drop').selectOption('disallow')
